...
def collect_rules():
    ...
    for rule in sorted(glob.glob("../rules/**/*.y*ml", recursive=True)) + sorted(
        glob.glob("../custom/rules/**/*.y*ml", recursive=True)
    ):
...
def output_baseline(
    rules, version, baseline_tailored_string, benchmark, authors, full_title
):
    inherent_rules = []
    permanent_rules = []
    na_rules = []
    ...
    for rule in rules:
        if "inherent" in rule.rule_tags:
            ...
            if rule.rule_id.startswith("system_settings"):
                section_name = (
                    rule.rule_id.split("_")[0] + "_" + rule.rule_id.split("_")[1]
                )
            else:
    ...
    if len(other_rules) > 0:
        for section in sections:
            output_text += '  - section: "{}"\n'.format(
                section_title(section, version["cpe"])
            )
            output_text += "    rules:\n"
            ...
            if rule.rule_odv == "missing":
                ...
                if benchmark == "recommended":
                    ...
                    if isinstance(rule.rule_odv["recommended"], int):
                        odv = sanitised_input(
                            f'Enter the ODV for "{rule.rule_id}" or press Enter for the recommended value ({rule.rule_odv["recommended"]}): ',
                            int,
                            default_=rule.rule_odv["recommended"],
                        )
                    ...
                    if isinstance(rule.rule_odv[benchmark], int):
                        odv = sanitised_input(
                            f'Enter the ODV for "{rule.rule_id}" or press Enter for the default value ({rule.rule_odv[benchmark]}): ',
                            int,
                            default_=rule.rule_odv[benchmark],
                        )
