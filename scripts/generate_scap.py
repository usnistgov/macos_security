...
def clean_oval_definition(oval_definition: str) -> str:
    """
    Cleans the OVAL definition by removing unwanted notes using regex.

    Args:
        oval_definition (str): The OVAL definition string to be cleaned.

    Returns:
        str: The cleaned OVAL definition string.
    """
...
def generate_scap(all_rules, all_baselines, args):
    ...
    output = "../build/macOS_{0}_Security_Compliance_Benchmark-{1}".format(
        version_yaml["os"], filenameversion
    )
    if "ios" in version_yaml["cpe"]:
        output = "../build/iOS_{0}_Security_Compliance_Benchmark-{1}".format(
            version_yaml["os"], filenameversion
        )
    if "visionos" in version_yaml["cpe"]:
        output = "../build/visionOS_{0}_Security_Compliance_Benchmark-{1}".format(
            version_yaml["os"], filenameversion
        )
    if export_as == "xccdf":
  ...
  <component id="scap_gov.nist.mscp.content_comp_macOS_{1}_xccdf.xml" timestamp="{0}">
    ...
    for rule in all_rules:
        ...
        for a in range(0, loop):
            ...
            try:
                ...
                rule_yaml["discussion"] = rule_yaml["discussion"].replace(
                    "$ODV", odv_value
                )
                ...
                if "result" in rule_yaml:
                    for result_value in rule_yaml["result"]:
                        if "$ODV" == rule_yaml["result"][result_value]:
                            rule_yaml["result"][result_value] = rule_yaml["result"][
                                result_value
                            ].replace("$ODV", odv_value)

                if rule_yaml["mobileconfig_info"]:
                    for mobileconfig_type in rule_yaml["mobileconfig_info"]:
                        if isinstance(
                            rule_yaml["mobileconfig_info"][mobileconfig_type], dict
                        ):
                                    ...
                                    if (
                                        ...
                                        for k, v in resulting_yaml["mobileconfig_info"][
                                            ...
                                            if v == "$ODV":
                                                resulting_yaml["mobileconfig_info"][
                                                    mobileconfig_type
                                                ][mobileconfig_value][k] = odv_value
                                    else:
                                        resulting_yaml["mobileconfig_info"][
                                            mobileconfig_type
                                        ][mobileconfig_value] = odv_value
            ...
            check_rule = str()
            if (
                "inherent" in rule_yaml["tags"]
                or "n_a" in rule_yaml["tags"]
                or "permanent" in rule_yaml["tags"]
            ):
                check_rule = """
            ...
            if (
                "800-53r5" in rule_yaml["references"]
                and rule_yaml["references"]["800-53r5"][0] != "N/A"
            ):
                references = (
                    references
                    + '<reference href="https://csrc.nist.gov/publications/detail/sp/800-53/rev-5/final">NIST SP 800-53r5: '
                )
                for nist80053 in rule_yaml["references"]["800-53r5"]:
                    references = references + nist80053 + ", "
                references = references[:-2] + "</reference>"
            if (
                "800-53r4" in rule_yaml["references"]
                and rule_yaml["references"]["800-53r4"][0] != "N/A"
            ):
                references = (
                    references
                    + '<reference href="https://csrc.nist.gov/publications/detail/sp/800-53/rev-4/final">NIST SP 800-53r4: '
                )
                for nist80053 in rule_yaml["references"]["800-53r4"]:
                    references = references + nist80053 + ", "
                references = references[:-2] + "</reference>"
            if (
                "800-171r3" in rule_yaml["references"]
                and rule_yaml["references"]["800-171r3"][0] != "N/A"
            ):
                references = (
                    references
                    + '<reference href="https://csrc.nist.gov/publications/detail/sp/800-171/rev-2/final">NIST SP 800-171r3: '
                )
                for nist800171 in rule_yaml["references"]["800-171r3"]:
                    references = references + nist800171 + ", "
                references = references[:-2] + "</reference>"
            if (
                "disa_stig" in rule_yaml["references"]
                and rule_yaml["references"]["disa_stig"][0] != "N/A"
            ):
                references = (
                    references
                    + '<reference href="https://public.cyber.mil/stigs/downloads/">DISA STIG(s): '
                )
            ...
            if "cis" in rule_yaml["references"]:
                if (
                    "benchmark" in rule_yaml["references"]["cis"]
                    and rule_yaml["references"]["cis"]["benchmark"][0] != "N/A"
                ):
                    references = (
                        references
                        + '<reference href="https://www.cisecurity.org/cis-benchmarks/">CIS Benchmark: '
                    )
                    for benchmark in rule_yaml["references"]["cis"]["benchmark"]:
                        references = references + benchmark + ", "
                    references = references[:-2] + "</reference>"
                if (
                    "controls v8" in rule_yaml["references"]["cis"]
                    and rule_yaml["references"]["cis"]["controls v8"][0] != "N/A"
                ):
                    references = (
                        references
                        + '<reference href="https://www.cisecurity.org/controls">CIS Controls V8: '
                    )
            ...
            cce = str()
            if (
                "cce" not in rule_yaml["references"]
                or rule_yaml["references"]["cce"] == "N/A"
            ):
                cce = "CCE-11111-1"
            else:
            ...
            if export_as == "scap":
                ...
                if rule_yaml["mobileconfig"]:
                    mobileconfig_info = escape(
                        format_mobileconfig_fix(rule_yaml["mobileconfig_info"])
                    )
                xccdf_rules = (
                    xccdf_rules
                    + """
            ...
            if "eficheck" in rule_yaml["check"]:
                ...
                continue
            if (
                "newsyslog.conf" in rule_yaml["check"]
                or "asl.conf" in rule_yaml["check"]
                or "aslmanager" in rule_yaml["check"]
            ):
                print(rule_yaml["id"] + " - Manual Check Required")
                xccdf_rules = replace_ocil(xccdf_rules, x)
                x += 1
            ...
            if "/usr/bin/pwpolicy getaccountpolicies" in rule_yaml["check"]:
                print(
                    rule_yaml["id"]
                    + " - pwpolicy getaccountpolicies - no relevant oval"
                )
                xccdf_rules = replace_ocil(xccdf_rules, x)
                x += 1
                continue
            if (
                "find" in rule_yaml["check"].split(" ")[0]
                and rule_yaml["id"] != "os_home_folders_secure"
            ):
                print(rule_yaml["id"] + " - no relevant oval")
                xccdf_rules = replace_ocil(xccdf_rules, x)
                x += 1
            ...
            </accountinfo_object>""".format(
                        rule_yaml["id"] + "_" + odv_label, x, x, x + 999, x + 999
                    )
                )
        ...
        <accountinfo_state xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" comment="accountinfo_state" id="oval:mscp:ste:{}" version="1">
                ...
                <login_shell operation="not equal">/usr/bin/false</login_shell>
                        ...
                        rule_yaml["id"] + "_" + odv_label, x, x + 999
                    )
                )
                ...
                for payload_type, info in rule_yaml["mobileconfig_info"].items():
                    ...
                    if payload_type == "com.apple.ManagedClient.preferences":
                        for payload_domain, settings in info.items():
                            ...
                            if len(settings) > 1:
                                oval_definition = (
                                    oval_definition + """<criteria operator="AND">"""
                                )
                            else:
                            ...
                            for key, value in settings.items():
                                ...
                                oval_definition = (
                                    oval_definition
                                    + """<criterion comment="{}" test_ref="oval:mscp:tst:{}" />""".format(
                                        rule_yaml["id"]
                                        + "_"
                                        + odv_label
                                        + "_"
            ...
            <concat>
                    ...
                    <literal_component datatype="string">/com.apple.dock.plist</literal_component>
                        ...
                        """.format(
                                ...
                                x += 1
                        oval_definition = (
                            oval_definition + """</criteria> </definition>"""
                        )
                        continue
        ...
        """.format(
                                ...
                                oval_object = (
                                    ...
                                    + """
            <plist511_object xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" comment="{}_object" id="oval:mscp:obj:{}" version="1">
                <filepath>/Library/Managed Preferences/{}.plist</filepath>""".format(
                                        rule_yaml["id"] + "_" + odv_label,
                                        x,
                                        payload_type,
                                    )
            ...
            <local_variable id="oval:mscp:var:{}" version="1" datatype="string" comment="user managed pref">
                <concat>
                    ...
                    <literal_component datatype="string">/com.apple.DiscRecording.plist</literal_component>
                            ...
                            continue
                        if (
                            payload_type == "com.apple.Safari"
                            and key == "AutoOpenSafeDownloads"
                        ):
                            oval_definition = (
                                oval_definition
                                + """
    ...
    <xpath>//*[contains(text(), "{}")]/following-sibling::*[1]/text()</xpath>
            ...
            <local_variable id="oval:mscp:var:{}" version="1" datatype="string" comment="user managed pref">
                <concat>
                    ...
                    <literal_component datatype="string">/com.apple.Safari.plist</literal_component>
                            ...
                            continue
                        if (
                            payload_type == "com.apple.systempreferences"
                            and key == "DisabledPreferencePanes"
                            or payload_type == "com.apple.systempreferences"
                            and key == "HiddenPreferencePanes"
                            or payload_type == "com.apple.systempreferences"
        ...
        """.format(
            ...
            <plist511_object xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" comment="{}_object" id="oval:mscp:obj:{}" version="1">
                <filepath>/Library/Managed Preferences/{}.plist</filepath>""".format(
                                rule_yaml["id"] + "_" + odv_label, x, payload_type
                            )
                        )
        ...
        <value_of>integrity_enabled</value_of>
                ...
                try:
                    ...
                    <criteria>
                        ...
                        <criterion comment="{}" test_ref="oval:mscp:tst:{}" />
                                ...
                                rule_yaml["id"] + "_" + odv_label,
                                rule_yaml["discussion"],
                                rule_yaml["id"] + "_standbydelayhigh",
                                x,
                                rule_yaml["id"] + "_standbydelaylow",
                                x + 877,
                                rule_yaml["id"] + "_highstandbythreshold",
                                x + 888,
                            )
                        )
                ...
                <plist511_test xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" check="all" check_existence="at_least_one_exists" comment="{}_test" id="oval:mscp:tst:{}" version="2">
                    ...
                    <state state_ref="oval:mscp:ste:{}" />
                                ...
                                rule_yaml["id"] + "_standbydelayhigh", x, x, x
                            )
                        )
            ...
            <local_variable id="oval:mscp:var:{}" version="1" datatype="string" comment="uuid variable">
                <concat>
                    ...
                    <literal_component datatype="string">.plist</literal_component>
                        ...
                        oval_object = (
                            ...
                            + """
                <plist511_object xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" comment="{}_object" id="oval:mscp:obj:{}" version="1">
                    <filepath datatype="string" operation="equals" var_check="at least one" var_ref="oval:mscp:var:{}"/>""".format(
                                rule_yaml["id"] + "_standbydelayhigh", x, x
                            )
                        )
                ...
                <plist511_object xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" comment="{}_object" id="oval:mscp:obj:{}" version="1">
                    <filepath datatype="string" operation="equals" var_check="at least one" var_ref="oval:mscp:var:{}"/>""".format(
                                rule_yaml["id"] + "_standbydelaylow", x + 877, x
                            )
                        )
                ...
                <plist511_object xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" comment="{}_object" id="oval:mscp:obj:{}" version="1">
                    <filepath datatype="string" operation="equals" var_check="at least one" var_ref="oval:mscp:var:{}"/>""".format(
                                rule_yaml["id"] + "_highstandbythreshold", x + 888, x
                            )
                        )
                    ...
                    <xpath>boolean(plist/dict[key="AC Power"]/dict[key="{}"]/integer/text() = "{}")</xpath>
                                ...
                                "Standby Battery Threshold", highstandbythreshold
                            )
                        )
                ...
                <value_of datatype="boolean" operation="equals">true</value_of>
                                ...
                                rule_yaml["id"] + "_standbydelaylow", x + 877
                            )
                        )
                ...
                <value_of datatype="boolean" operation="equals">true</value_of>
                                ...
                                rule_yaml["id"] + "_highstandbythreshold", x + 888
                            )
                        )
        ...
        """.format(
                ...
                </textfilecontent54_object>""".format(
                                x, rule_yaml["id"] + "_" + odv_label, check_string
                            )
                        )
                ...
                <instance datatype="int">1</instance>
                    ...
                    if "awk" in rule_yaml["check"].split("|")[1]:
                        if (
                            "timestamp_type" in rule_yaml["fix"]
                            and rule_yaml["result"]["string"] == "tty"
                        ):
                            oval_definition = (
                                oval_definition
                                + """
            ...
            """.format(
                    ...
                    <instance datatype="int">1</instance>
                                    ...
                                    x + 8000, rule_yaml["id"] + "_" + odv_label
                                )
                            )
                    ...
                    <instance datatype="int">1</instance>
                                    ...
                                    x + 8001, rule_yaml["id"] + "_" + odv_label
                                )
                            )
                            oval_object = (
                                oval_object
                    ...
                    <instance datatype="int">1</instance>
                                    ...
                                    x + 8002, rule_yaml["id"] + "_" + odv_label
                                )
                            )
                            x = x + 1
                            continue
                        else:
                            check_string = "Defaults.*.timestamp_type={}".format(
                                rule_yaml["result"]["string"]
                            )
            ...
            """.format(
                ...
                if (
                    "ssh_config" in rule_yaml["discussion"]
                    and "dscl" in rule_yaml["check"]
                ):
                    oval_definition = (
                        oval_definition
                        + """
        ...
        """.format(
                ...
                <instance datatype="int">1</instance>
                            ...
                            x, rule_yaml["id"] + "_" + odv_label, ssh_config_pattern
                        )
                    )
            ...
            <accountinfo_object xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" comment="user home directory" id="oval:mscp:obj:{}" version="1">
                ...
                <literal_component datatype="string">/.ssh/config</literal_component>
                    ...
                    continue
                if (
                    "sshd -T" in rule_yaml["check"]
                    and "fips" in rule_yaml["check"]
                    or "sshd -G" in rule_yaml["check"]
                    and "fips" in rule_yaml["check"]
                ):
                    fipslist = (
                        rule_yaml["check"]
        ...
        """.format(
                ...
                <instance datatype="int">1</instance>
                            ...
                            x, rule_yaml["id"] + "_" + odv_label, fipslist
                        )
                    )
                ...
                <instance datatype="int">1</instance>
                            ...
                            x + 6000, rule_yaml["id"] + "_" + odv_label, fipslist
                        )
                    )
        ...
        """.format(
                    ...
                    if "awk" in rule_yaml["check"]:
                        ...
                        for item in rule_yaml["result"]:
                            sshd_config_pattern = (
                                matchy_match + " " + str(rule_yaml["result"][item])
                            )
                ...
                <instance datatype="int">1</instance>
                            ...
                            x, rule_yaml["id"] + "_" + odv_label, sshd_config_pattern
                        )
                    )
                ...
                <plist511_test xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" check="all" check_existence="at_least_one_exists" comment="{}_test" id="oval:mscp:tst:{}" version="2">
                    ...
                    <state state_ref="oval:mscp:ste:{}" />
                        ...
                        oval_object = (
                            ...
                            + """
                <plist511_object xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" comment="{}_object" id="oval:mscp:obj:{}" version="1">
                    <filepath>/Library/Preferences/com.apple.PowerManagement.plist</filepath>""".format(
                                rule_yaml["id"] + "_" + odv_label, x
                            )
                        )
                        pmset_key = str()
            ...
            """.format(
                ...
                <xpath>//*[contains(text(), "{}")]/following-sibling::*[1]/text()</xpath>
                            ...
                            rule_yaml["id"] + "_" + odv_label, x, firewall_variable
                        )
                    )
            ...
            <value_of datatype="int" operation="equals">1</value_of>
                ...
                {}
                </systemsetup_state>""".format(
                                rule_yaml["id"] + "_" + odv_label, x, state_test
                            )
                        )
        ...
        <concat>
            ...
            <literal_component datatype="string">plist</literal_component>
                        ...
                        <filepath>{}</filepath>""".format(
                                rule_yaml["id"] + "_" + odv_label, x, plist
                            )
                        )
                    ...
                    if (
                        oval_datatype == "boolean"
                        and rule_yaml["result"][datatype] == 0
                    ):
                        value = "false"
                    elif (
                        oval_datatype == "boolean"
                        and rule_yaml["result"][datatype] == 1
                    ):
                        value = "true"
                    else:
            ...
            <value_of datatype="{}" operation="equals">{}</value_of>
                            ...
                            rule_yaml["id"] + "_" + odv_label, x, oval_datatype, value
                        )
                    )
                ...
                <authorizationdb_test xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" check="all" check_existence="only_one_exists" comment="{}_test" id="oval:mscp:tst:{}" version="2">
                    ...
                    <value_of>{}</value_of>
                                        ...
                                        rule_yaml["id"] + "_" + odv_label, x, key
                                    )
                                )
                            ...
                            if "authorizationdb" in rule_yaml["check"]:
                                ...
                                matchy_match = []
                                matches = re.finditer(
                                    regex, rule_yaml["check"], re.MULTILINE
                                )
                                for matchNum, match in enumerate(matches, start=1):
                                    matchy_match = (
                                        match.group()
            ...
            <file_object id="oval:mscp:obj:{}" version="1" comment="{}_object" xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#unix">
                ...
                <filename xsi:nil="true" />
                            ...
                            x, rule_yaml["id"] + "_" + odv_label, path.rstrip()
                        )
                    )
                    x += 1
                    continue
                ...
                <file_object id="oval:mscp:obj:{}" version="1" comment="{}_rtfd_object" xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#unix">
                    ...
                    <filename xsi:nil="true" />
                        ...
                        if "grep" in s.split()[2]:
                            ...
                            config_file = substring = grep_search.split("|")[0].split()[
                                -1
                            ]
                ...
                """.format(
                    ...
                    <object_component object_ref="oval:mscp:obj:{}" item_field="subexpression"/>
                                    ...
                                    x, rule_yaml["id"] + "_" + odv_label, x + 999
                                )
                            )
            ...
            """.format(
                ...
                <file_test id="oval:mscp:tst:{}" version="1" comment="{}_test" check_existence="all_exist" check="all" xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#unix">
                    ...
                    <state state_ref="oval:mscp:ste:{}"/>
                        ...
                        if (
                            "-" in fix_command
                            and "R" in fix_command
                            or rule_yaml["fix"].split("\n")[2][-1] == "*"
                        ):
                    ...
                    <filepath datatype="string" operation="equals">{}</filepath>
                        ...
                        if (
                            "-" in fix_command
                            and "N" in fix_command
                            and "chmod" in fix_command
                        ):
                            state_test = """
                ...
                <oexec datatype="boolean">true</oexec>"""
                        ...
                        oval_state = (
                            oval_state
                            + """
                <file_state xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#unix" version="1" comment="{}_state" id="oval:mscp:ste:{}">""".format(
                                rule_yaml["id"] + "_" + odv_label, x
                            )
                            + state_test
                            + """
            ...
            """.format(
                ...
                try:
                    if "launchctl" in command[2] or "launchctl" in rule_yaml["fix"]:
                        if (
                            "disable" in command[2]
                            and "=> true" in rule_yaml["check"]
                            or "unload -w" in rule_yaml["fix"]
                            or "disable" in command[2]
                            and "=> disabled" in rule_yaml["check"]
                        ):
                            oval_definition = (
                                oval_definition
                                + """
                ...
                <plist511_state xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" comment="{}_plist_state" id="oval:mscp:ste:{}" version="1">
                    <value_of datatype="boolean" operation="equals">{}</value_of>
                                    ...
                                    rule_yaml["id"] + "_" + odv_label, x, status
                                )
                            )
    ...
    for k in generated_baselines.keys():
        ...
        <Profile id="xccdf_gov.nist.mscp.content_profile_{1}">
                <title>{0}</title>
                <description>This profile selects all rules tagged as {0}.</description>""".format(
                k, k.replace(" ", "_")
            )
        )
        for v in generated_baselines[k]:
            xccdf_profiles = (
                xccdf_profiles
                + """
                <select idref="xccdf_gov.nist.mscp.content_rule_{0}" selected="true"/>""".format(
                    v
                )
            )
        xccdf_profiles = (
...
""".format(
    ...
    final_oval = clean_oval_definition(total_oval)
    # final_oval = re.sub("(?=\n\\[NOTE\\])(?s)(.*)\=\n$.*", "<", total_oval)
      ...
      <states>
        ...
        <plist511_state xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#macos" comment="Is the value is greater than or equal to {2}" id="oval:gov.nist.mscp.content.cpe:ste:2" version="1">
          <value_of datatype="version" operation="greater than or equal">{2}</value_of>
            ...
            date_time_string, version_yaml["cpe"], version_yaml["os"]
        )
    )
    scap_file = output
...
def collect_rules():
    ...
    for rule in glob.glob("../rules/**/*.yaml", recursive=True) + glob.glob(
