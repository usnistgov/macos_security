...
The mapping script creates custom rules and baselines for compliance frameworks not included in the project. You provide a CSV that maps your framework's controls to a supported framework (like NIST 800-53r5).
...
1. **Run the mapping command**

   ```bash
   ./mscp.py mapping -c PATH_TO_CSV
   ```

   Example:
   ```bash
   ./mscp.py mapping -c ~/Desktop/171-to-53.csv
   ```
...
```bash
./mscp.py mapping -c ~/Desktop/cis-mapping.csv -f "cis/controls v8"
```
