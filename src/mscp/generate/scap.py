...
from .baseline import print_keyword_summary, collect_tags_and_benchmarks, rule_has_benchmark_for_version
...
def generate_scap(args: argparse.Namespace) -> None:
    ...
    if args.baseline == "all_rules":
        ...
        for v in all_tags:        
            if v == "n_a" or v == "inherent" or v == "manual" or v == "none" or v == "permanent" or v == "supplemental" or v == "i386" or v == "arm64" or v == "srg":
                continue
    ...
    for baseline in all_the_baselines:
        for b,r in baseline.items():
            xccdfrules = str()
            xccdfProfiles = xccdfProfiles + '''<Profile id="xccdf_gov.nist.mscp.content_profile_{0}"><title>{0}</title><description>This profile selects all rules tagged as {0}.</description>'''.format(b)
            for rule in r:  
                odv_tag = "recommended"
                ...
                xccdfProfiles = xccdfProfiles + '''<select idref="xccdf_gov.nist.mscp.content_rule_{0}_{1}" selected="true"/>'''.format(rule['rule_id'],odv_tag)
    ...
    for rule in all_rules:
        ...
        if args.baseline != "all_rules":
            if not rule_has_benchmark_for_version(rule, args.baseline, args.os_name, str(args.os_version)) and args.baseline not in rule.tags:                
                continue
        ...
        try:
            if len(rule['references'].nist.nist_800_53r5) > 0:
                xccdf_references = xccdf_references + '''<reference href="https://csrc.nist.gov/publications/detail/sp/800-53/rev-5/final">NIST SP 800-53r5: {0}</reference>'''.format(separator.join(rule['references'].nist.nist_800_53r5))
        except:
            pass
        try:
            if len(rule['references'].nist.nist_800_171r3) > 0:
                xccdf_references = xccdf_references + '''<reference href="https://csrc.nist.gov/pubs/sp/800/171/r3/final">NIST SP 800-171r3: {0}</reference>'''.format(separator.join(rule['references'].nist.nist_800_171r3))
        except:
            pass
        try:
            if len(rule['references'].disa.disa_stig) > 0:
                xccdf_references = xccdf_references + '''<reference href="https://www.cyber.mil/stigs/downloads/">DISA STIG(s): {0}</reference>'''.format(separator.join(rule['references'].disa.disa_stig))
        except:
            pass
        try:
            if len(rule['references'].cis.benchmark) > 0:
                xccdf_references = xccdf_references + '''<reference href="https://www.cisecurity.org/cis-benchmarks/">CIS Benchmark: {0}</reference>'''.format(separator.join(rule['references'].cis.benchmark))
        except:
            pass
            ...
            for k,v in rule.odv.items():
                ...
                if k in selected_os_benchmark:
                    ...
                    if args.xccdf == None and args.oval == None:
                        check_content = '''<check system="http://oval.mitre.org/XMLSchema/oval-definitions-5"><check-content-ref href="oval.xml" name="oval:mscp:def:{}"/></check>'''.format(oval_counter)
                    rule._fill_in_odv(k)
                            ...
                            if "/usr/bin/ssh -G ." not in check_value:
                                if "auditd_enabled" not in check_value:
                                    if "/usr/sbin/sshd -G" not in check_value:
                                        ...
                                        check_value = check_value.replace("/usr/bin/grep -c ", "/usr/bin/grep ")
                                        count_found = True
                    ...
                    if "launchctl list" in check_value:
                        check_value = check_value.replace("launchctl list", "launchctl print system")
                        if "auditd_enabled" in check_value:
                            check_value = check_value.replace("/usr/bin/grep -c com.apple.auditd", "/usr/bin/grep -c '\"com.apple.auditd\" => enabled'")
...
{}'''.format(check_value)   
                    ...
                    xccdfrules = xccdfrules + '''<Rule id="xccdf_gov.nist.mscp.content_rule_{0}_{1}" selected="false" role="full" severity="{2}" weight="1.0"><title>{3}</title><description>{4}
                {5}

                Expected Result: {6}
                </description>{7}<ident system="https://ncp.nist.gov/cce">{8}</ident><fixtext>        {9}
                </fixtext>{10}
                </Rule>'''.format(rule['rule_id'],k, rule.severity, rule['title'], escape(rule['discussion']), check_value, rule.result_value, xccdf_references, rule['references'].nist.cce, fix_value, check_content)            
                
                if args.os_name == "macos":
                
                    oval_def = oval_def + '''<definition id="oval:mscp:def:{0}" version="1" class="compliance"><metadata><title>{1}</title><reference source="CCE" ref_id="{2}"/><reference source="macos_security" ref_id="{3}_{4}"/><description>{5}</description></metadata><criteria><criterion comment="{3}_{4}" test_ref="oval:mscp:tst:{0}"/></criteria></definition>'''.format(oval_counter, rule['title'], rule['references'].nist.cce, rule['rule_id'], k, escape(rule['discussion']))
                    
                    oval_tests = oval_tests + '''<shellcommand_test xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#independent" id="oval:mscp:tst:{0}" version="1" comment="{1}_{2}_test" check_existence="{3}" check="all"><object object_ref="oval:mscp:obj:{0}"/><state state_ref="oval:mscp:ste:{0}"/></shellcommand_test>'''.format(oval_counter,rule.rule_id,k, check_existance)
                    
                    oval_objects = oval_objects + '''<shellcommand_object xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#independent" id="oval:mscp:obj:{0}" version="1" comment="{1}_{2}_object"><shell>zsh</shell><command>{3}</command></shellcommand_object>'''.format(oval_counter, rule.rule_id, k, check_value)
                    
                    if count_found:
                        if check_existance != "none_exist":
                            oval_states = oval_states + '''<shellcommand_state xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#independent" id="oval:mscp:ste:{0}" version="1" comment="{1}_{2}_state"><stdout_line operation="pattern match">.*</stdout_line></shellcommand_state>'''.format(oval_counter, rule.rule_id,k)
                        else:
                            oval_states = oval_states + '''<shellcommand_state xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#independent" id="oval:mscp:ste:{0}" version="1" comment="{1}_{2}_state"><stdout_line check_existence="none_exist" /></shellcommand_state>'''.format(oval_counter, rule.rule_id,k)
                        
                    else:
                        oval_states = oval_states + '''<shellcommand_state xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#independent" id="oval:mscp:ste:{0}" version="1" comment="{1}_{2}state"><stdout_line operation="equals">{3}</stdout_line></shellcommand_state>'''.format(oval_counter, rule.rule_id,k,rule.result_value)    
...
{}'''.format(check_value)
            ...
            xccdfrules = xccdfrules + '''<Rule id="xccdf_gov.nist.mscp.content_rule_{0}_{1}" selected="false" role="full" severity="{2}" weight="1.0"><title>{3}</title><description>{4}
                {5}
                Expected Result: {6}
                </description>{7}<ident system="https://ncp.nist.gov/cce">{8}</ident><fixtext>{9}</fixtext>{10}</Rule>'''.format(rule['rule_id'],"recommended", rule.severity, rule['title'], escape(rule['discussion']), check_value, rule.result_value, xccdf_references, rule['references'].nist.cce, fix_value, check_content)

            if args.os_name == "macos":
                
                oval_def = oval_def + '''<definition id="oval:mscp:def:{0}" version="1" class="compliance"><metadata><title>{1}</title><reference source="CCE" ref_id="{2}"/><reference source="macos_security" ref_id="{3}_{4}"/><description>{5}</description></metadata><criteria><criterion comment="{3}_{4}" test_ref="oval:mscp:tst:{0}"/></criteria></definition>'''.format(oval_counter, rule['title'], rule['references'].nist.cce, rule['rule_id'], "recommended", escape(rule['discussion']))
                    
                oval_tests = oval_tests + '''<shellcommand_test xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#independent" id="oval:mscp:tst:{0}" version="1" comment="{1}_{2}_test" check_existence="{3}" check="all"><object object_ref="oval:mscp:obj:{0}"/><state state_ref="oval:mscp:ste:{0}"/></shellcommand_test>'''.format(oval_counter,rule.rule_id,"recommended", check_existance)
                    
                oval_objects = oval_objects + '''<shellcommand_object xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#independent" id="oval:mscp:obj:{0}" version="1" comment="{1}_{2}_object"><shell>zsh</shell><command>{3}</command></shellcommand_object>'''.format(oval_counter, rule.rule_id, "recommended", check_value)
                    
                if count_found:
                    if check_existance != "none_exist":
                        oval_states = oval_states + '''<shellcommand_state xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#independent" id="oval:mscp:ste:{0}" version="1" comment="{1}_{2}_state"><stdout_line operation="pattern match">.*</stdout_line></shellcommand_state>'''.format(oval_counter, rule.rule_id,"recommended")
                    else:
                        oval_states = oval_states + '''<shellcommand_state xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#independent" id="oval:mscp:ste:{0}" version="1" comment="{1}_{2}_state"><stdout_line check_existence="none_exist" /></shellcommand_state>'''.format(oval_counter, rule.rule_id,"recommended")
                    
                else:
                    oval_states = oval_states + '''<shellcommand_state xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#independent" id="oval:mscp:ste:{0}" version="1" comment="{1}_{2}state"><stdout_line operation="equals">{3}</stdout_line></shellcommand_state>'''.format(oval_counter, rule.rule_id,"recommended",rule.result_value)                                    
    ...
    if args.os_name != "macos" and args.oval == None and args.xccdf == None:
        logger.error("SCAP generation is only avalilable for MacOS")
        sys.exit()
    ...
    if args.oval == None and args.xccdf == None and args.os_name == 'macos':
        ...
        totaloutput = pretty_format_xml(scap + xccdf + oval + ocil + cpe + "</data-stream-collection>")
