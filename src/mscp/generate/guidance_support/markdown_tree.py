...
Frontmatter is kept minimal — ``title`` and ``slug`` only — so it works across
all generators.  Docusaurus-specific sidebar metadata (``sidebar_position``,
...
def generate_markdown_tree(
    ...
    Frontmatter is minimal — ``title`` and ``slug`` only — and is safe to use
    in any CommonMark-based static site generator.  The rendered text is MDX-
    ...
    (output_root / "index.md").write_text(index_page)
    ...
    for section_position, profile in enumerate(baseline.profile, start=2):
        ...
        (section_dir / "index.md").write_text(section_index)
