...
_PLATFORM_RE = re.compile(r"\b(macOS|iOS|iPadOS|visionOS)\s+(\d+(?:\.\d+)*)\b")
...
class LegacyBaseline(BaseModel):
    ...
    def parse_authors(self) -> list[Author]:
        ...
        Recognizes the AsciiDoc simple table format used by early mSCP
        baselines::
        ...
        marked ``additional=True``, matching the behavior of tailored
    ...
    def parse_platform(self) -> dict[str, Any]:
        ...
        Raises:
            ValueError: If no recognizable platform string is found.  The
    ...
    def migrate(
        ...
        Args:
            ...
            platform (dict[str, Any] | None): Target platform as
                ...
                recognizable platform string, or to override the inferred
                value.
    ...
    def migrate_file(
        ...
        Returns:
            list[str]: Rule IDs not found in the current library (see
                `migrate`).
        """
        return cls.from_yaml(source).migrate(
            output, platform=platform, language=language
        )
