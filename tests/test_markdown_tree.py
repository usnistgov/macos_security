...
def _get_baseline_path() -> Path:
    ...
    for p in candidates:
        if p.exists():
            return p
    pytest.skip("No bundled baseline found for integration test")
...
class TestMarkdownTreeIntegration:
    ...
    def test_rule_frontmatter_parses(self, markdown_tree_output):
        """All rule files must have valid YAML frontmatter with title and slug."""
        for section_dir in markdown_tree_output.iterdir():
            if not section_dir.is_dir():
                continue
            for rf in section_dir.iterdir():
                ...
                assert "slug:" in fm_block, f"{rf.name}: missing slug"
